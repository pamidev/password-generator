...
def password_generator(pass_length: int = 12):
    ...
    while True:
        ...
        if (any(char in string.punctuation for char in gen_pass) and
                sum(char in string.digits for char in gen_pass) >= 3 and
                sum(char in string.ascii_lowercase for char in gen_pass) >= 3 and
                sum(char in string.ascii_uppercase for char in gen_pass) >= 3):
            break
    return gen_pass
